...
export const createOrEditTemplateSchemaValidation = () => {
  ...
  if (select?.category?.name !== "auto_post_views")
    defaultFields.count = yup
      ...
      .min(
        select?.["min_count"],
        `Количество не должно быть меньше чем ${select?.["min_count"]}`,
      );

  // Валидация, если есть поле с комментариями
  if (comments(select?.option, "templates"))
  ...
  if (pollVoteAnswers(select?.option, "templates"))
    defaultFields[pollVoteAnswersName(select?.option, "templates")] = yup
      .string()
      .required("Поле должно быть заполнено!");
  ...
  if (slider(select?.option, "templates"))
    defaultFields[sliderName(select?.option, "templates")] = yup
      ...
      .required("Поле должно быть заполнено!")
      .min(
        sliderMin(select?.option, "templates"),
        `Удержание не должно быть больше чем ${sliderMin(select?.option, "templates")}`,
      )
      .max(
        sliderMax(select?.option, "templates"),
        `Удержание не должно быть меньше чем ${sliderMax(select?.option, "templates")}`,
      );
  ...
  if (deleteSelect === null) {
    const createData = {
      ...
      services: isToggleMode
        ...
        : [
            ...utilTransformServices(template.services),
            ...utilTransformResponseAddService(select, data),
          ],
    ...
    saveTemplate({
      ...
      dispatch(
        profileTemplatesApi.util.invalidateTags(["getWebsiteDetailedServices"]),
      );
      ...
      if (elem.uuid === deleteSelect.uuid) {
        return {
          ...
          option: elem.option.map((item) => {
            return {
              ...item,
              data: `${elem?.category.name}`.includes("auto_")
 ...
 * @returns {unknown[]}
 */
export const utilRenderServicesCreateOrEditTemplate = (
  services,
  isMobile,
  serviceName,
  handleDeleteService,
  ...
  return (
    ...
    services?.map((elem, index) => {
      ...
      const iconService = !isMobile
        ? servicesConfig?.[`${serviceName}`]?.icon
        : servicesConfig?.[`${serviceName}`]?.iconNotSocial;
      const bgService = servicesConfig?.[`${serviceName}`]?.bg;
      const classIcon = classnames(
        "max-sm:w-[20px] max-sm:flex max-sm:items-center max-sm:justify-center max-sm:rounded-[8px]",
        {
          [`${bgService}`]: isMobile,
        },
      );
      ...
      return (
        <div
          className="flex py-3 border-b w-full items-center max-sm:items-stretch sm:gap-4 gap-3 border-gray-200"
          key={uuid}
        >
          {/* Иконка */}
          ...
          <div className="flex items-center w-full max-sm:gap-2 max-sm:flex-row max-sm:items-start max-sm:flex-wrap max-sm:justify-between">
            ...
            <div className="flex items-center max-sm:flex-wrap sm:gap-2 max-sm:gap-x-1 max-sm:w-[70%]">
              <p className="text-sm sm:text-[16px] font-pn-semibold text-gray-600">
                {categoryRuName}
              </p>
            ...
            <div className="flex sm:ml-5 sm:gap-5 max-sm:w-full">
              ...
              >
                <p className="sm:hidden text-gray-500 font-pn-regular text-sm">
                  Редактировать
                </p>
              ...
              >
                ...
                <p className="sm:hidden text-red-500 font-pn-regular text-sm">
                  Удалить
                </p>
              </div>
            </div>
...
export const utilTransformResponseAddService = (select, data) => {
  const isCategoryNameAutoPostViews = `${select?.category?.name}`.includes(
    "auto_",
  );
    ...
    (Array.isArray(select.option) &&
      select.option?.map((opt) => {
        if (opt.name === "comments")
          return {
            data:
              isCategoryNameAutoPostViews && opt.name === "count"
                ? 10
                : data[opt.name],
            name: opt.name,
          };
