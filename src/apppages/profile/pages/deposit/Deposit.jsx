...
import { useYupValidationResolver, useBrowserChecker, useTypeDevice } from "@hooks";
import { useForm } from "react-hook-form";
import { useSelector } from "react-redux";
import { RHFProvider } from "@providers";
import { useEffect } from "react";

import { DepositRefill, DepositTable, DepositPromo } from "./components";
import { validationSchemeDeposit } from "./utils";
import "./assets/styles/deposit.scss";
...
const Deposit = () => {
    ...
    useEffect(() => {
        if (data) {
            if (isSafari || isFirefox) window.location.href = data.data["redirect_to"];
    ...
    return (
        <RHFProvider onSubmit={handleSubmit(onSubmit)} className="deposit__content" onKeyPress={handleKeyDown} methods={methods}>
            {/* Пополнение баланса */}
            <DepositRefill />
