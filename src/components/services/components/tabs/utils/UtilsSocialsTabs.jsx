...
export const UtilsSocialsTabs = () => {
  ...
  const classItem = (elem) =>
    classNames(
      ...
      {
        pseudoOutlineSafari: service === elem.name,
      },
      elem.bg,
    );

  const filteredConfigSocialTabsDataServices =
    configSocialTabsDataServices?.filter((elem) => {
  ...
  return (
    <>
      {filteredConfigSocialTabsDataServices?.map((elem, index) => (
        <Link
